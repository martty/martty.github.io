...
Trap handlers are quite simple, the important bits to note are that the hardware prefills some `ttmp` registers for us on entering. For our current purposes, we only care about 

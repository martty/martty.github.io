...
Of course, we now need to enlist the help of the driver - we want to alter the state of the debugee. So lets search for the most appropriate place to do this in `radv` and
...
Except the relevant code in `radv` is assert-gated behind `gfx8`, however I want to run this on `gfx10.3`. 
...
I suppose that is a testament to the number of users for this so far. But no matter, with that fixed we try to enable the trap handler - and nothing happens. This is too anticlimactic, so lets do some more research. It turns out Sam Pitoiset has done more work on this: [a second MR for gfx8+](https://gitlab.freedesktop.org/mesa/mesa/-/merge_requests/6488). And this MR comes with a related
...
(what the first mesa MR enables). However of `gfx9` and above, setting `TBA` and `TMA` is a privileged operation and only the kernel can do it. And currently it doesn't.
...
What this means for us is that if the trap handler installation requires setting some registers, we can probably do it ourselves instead having to get a new kernel!

## Banked registers

An important aspect of setting registers is banking. The GPU can have many of a certain type of register, organised into banks. There are two banking mechanisms important for us: `GRBM`, which select by `SE`, `SA` and `INSTANCE` 
(which is a combination of WGP and SIMD indices), and `SRBM` which selects based `ME`, `PIPE` and `QUEUE`[^SRBM]. When we perform a banked register read or write, `umr` tells the KMD to first set the `GRBM` or `SRBM`, thereby activating the 
bank we want, then performs the access to get a specific register. For writes it is also possible to have them broadcast into multiple banks. The reason we need a bit of dance is because the `GRBM` and `SRBM` registers are global -
another process (or the kernel itself) could change simulataneously, and then our write will go to the wrong bank. For this reason we need the KMD to have a lock to facilitate race-free MMIO.
...
[^SRBM]: another way of looking at it is that GRBM-banked registers are per SIMD unit, and SRBM-banked registers are per ring.

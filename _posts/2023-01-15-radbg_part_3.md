...
To install the trap handler, we now need to enlist the help of the driver, since we want to alter the state of the debugee. So let's search for the most appropriate place to do this in `radv` and
...
Except the relevant code for trap handlers in `radv` is gated behind asserts requiring `gfx8` or lower, however I want to run this on `gfx10.3`. 
Lets see what happens if we just remove the asserts and let it run anyway: baby's first mesa [MR](https://gitlab.freedesktop.org/mesa/mesa/-/merge_requests/20354) for fixing an uninitialized struct on the trap handler path.
...
```isa
; depending on how many waves are running per frame, 
; we might need more or less sleeping for the frame time to be significantly different
